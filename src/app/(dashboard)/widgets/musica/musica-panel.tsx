...
import { SkipForward, Pause, Play, Trash2, Music, PlayCircle } from "lucide-react";
...
export function MusicaPanel({ widgetUrl, remoteBase, queue }: Props) {
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="rounded-2xl border border-white/10 bg-pixflow-darker/50 p-5">
        <p className="font-medium text-pixflow-slate">Widget de pedido de música (OBS)</p>
        <p className="mt-1 text-xs text-white/40">
          No OBS: Fontes → + → Navegador → cole a URL. É só um card visual —
          toque a música manualmente no seu app (Spotify etc).
        </p>
      ...
      {/* Controles da fila */}
      <div className="rounded-2xl border border-white/10 bg-pixflow-darker/50 p-5">
        <p className="mb-3 font-medium text-pixflow-slate">Fila de músicas</p>
        <div className="flex flex-wrap gap-2">
          <Button
            variant="ghost"
            onClick={() => run("clear", () => clearMediaQueue("MUSIC"))}
      ...
      <div className="rounded-2xl border border-white/10 bg-pixflow-darker/50 p-5">
        ...
        <div className="mt-3 grid gap-2">
          {[
            { label: "Pular música atual", icon: SkipForward, path: "skip" },
      ...
      <div className="rounded-2xl border border-white/10 bg-pixflow-darker/50 p-5">
        ...
        {queue.length === 0 ? (
          <EmptyState icon={Music} title="Nenhum pedido de música ainda" />
        ) : (
